...
help :-
   ...
   send_list( Editor, append,
	 ['Aristotle - A system for Computational Argumentation and defeasible reasoning\n\n',
	  'Rule syntax:\n',
	     '\tRule => Fact | Label Conditions then Conclusion.\n',
	     '\tLabel => [] | Identifier [:]\n',
	     '\tIdentifier => [a-z]+[0-9]*\n',
	     '\tConditions => true | Fact | Fact and Conditions\n',
	     '\tConclusion => Fact\n\n',
	  ...
	  'Dialogue example  (file name extension DKB):\n\n',
	     '\tSHe believes chicken(tina).                   % They believe in a fact\n',
             '\tSHe believes chicken(X) then bird(X).	      % They believe in a rule\n',
	     '\tpaul believes p1: bird(X) then fly(X).        % Paul\'s belief\n',
             '\tolga believes o1: chicken(X) then not fly(X). % Olga\'s belief\n',

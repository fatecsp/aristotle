...
socrates :-
   ...
   new( QuitButton,                 button( quit,                message( Frame,   destroy ) ) ),
...
/*----------------------------------------------------------------+
|		        PERSUASION DIALOGUE                       |
| the agent assumes both roles in the dialogue (i.e., pro and con)|
+----------------------------------------------------------------*/
...
% load: get Clauses from updateKB
% and set the precedence relation to be used.

/*
loadkb :-
   findall(R:A then C,updateKB:then(R,A,C),Clauses),
   nb_setval(kb,Clauses),
   set_type.
*/
